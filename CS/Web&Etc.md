...
(플랫폼은 사실 아주 오래전부터 인류가 실행해 온 개념입니다. 
하루아침에 벼락스타가 된 건 아니죠. ‘생산자와 소비자가 모여 상호작용할 수 있는 
공간을 만들어 서로를 위한 가치를 창출하게 하자는 것’이 플랫폼의 기본 정의입니다. 
매우 간단하죠. 상인들이 자기들의 물건을 가져와서 지역 소비자들에게 파는 전통적인 
노천시장을 떠올려보면 플랫폼의 의미를 좀 더 직관적으로 이해할 수 있습니다.

  플랫폼은 소비자와 생산자가 가치를 생산할 수 있는 공간을 의미하는 것같다.
  ex)배달의 민족, 우버 택시
)
...
#### 6 - 3. @Override 키워드 사용 이유

```
1) 일종의 안전장치입니다. 예컨대 어떤 라이브러리에 속한 클래스를 상속받아 특정 메서드를 오버라이드했는데, 그 라이브러리가 업데이트 되면서 해당 메서드의 시그네쳐가 바뀌는 경우를 생각할 수 있습니다.
...
2) 이 메소드가 오버라이드 되어 있다는 바로 알수 있게 해주는 용도로 사용도 가능합니다.

그럼 이 클래스 구조를 인식하기 좋겠죠. 
어노테이션 없다면 이게 오버라이딩 한것인지 여부를 상위 클래스 가서 찾아야 하는데
단순 추상 클래스(또는 인터페이스) 하나만 있다면 상관없지만 
여러가지 상속 깊이가 있는 경우에는 바로 찾아서 확인하기도 힘듭니다. 
그 때 @Override 어노테이션 알 수 있으니 좋은거죠
...
[![image](https://user-images.githubusercontent.com/49560745/72142264-a8ba7a80-33d7-11ea-9826-6d8760550bab.png)](https://user-images.githubusercontent.com/49560745/72142264-a8ba7a80-33d7-11ea-9826-6d8760550bab.png)
...
[![image](https://user-images.githubusercontent.com/49560745/77242906-0294b780-6c47-11ea-8d36-030a7ea0c151.png)](https://user-images.githubusercontent.com/49560745/77242906-0294b780-6c47-11ea-8d36-030a7ea0c151.png)
...
[![image](https://user-images.githubusercontent.com/49560745/76371750-a09e9d00-637e-11ea-9b1d-47c383588729.png)](https://user-images.githubusercontent.com/49560745/76371750-a09e9d00-637e-11ea-9b1d-47c383588729.png)
...
-----------------------------------------------------------------------------------------
* 마크업(Mark Up) 언어란?
"마크로 둘러싸인" 언어이다. "태그(Tag)"로 둘러싸인 언어라고도 표현한다.
HTML,XML 등의 마크업 언어들은 문서의 구조를 정의한다. 쉽게 말해 문서의 골격부분에 해당하는 부분을 작성하는데 활용한다.

* 마크다운(Makr Down) 언어란 ?
    마크업언어의 일종으로 읽기도 쓰기도 쉽다는 장점이있다.
    typora는 마크다운 언어를 사용하는 툴이라고 볼 수 있고, .md의 확장자를 갖는다.

* HTML은 프론트엔드의 뼈대를 만들어준다.
  사람으로 비유하자면 사람의 형태라고 할 수 있다.

  CSS는 프론트엔드를 꾸며주는 역할을 한다.
  사람이 입는 옷이 이 CSS라고 볼 수 있다.

  javascript는 프론트엔드의 동적인 움직임을 제어한다.
  따라서, 사람이 팔을벌리거나 걷거나, 뛰는 것에 비유할 수 있다.


* XHTML
XHTML은 HTML 규격이 가진 문제점을 극복하고, 보다 다양한 분야에 응용될 수 있도록 해주는 여러가지 확장된 기능을 포함하고 있다.
...
3) DOM 및 CSSOM을 결합하여 렌더링 트리를 형성한다.("화면에 그렬질 것만" 결정)

-----------------------------------------------------------------------------------------
	* DOM은 웹 페이지에 대한 프로그래밍 인터페이스(APIT)이다.
      CSSOM은 CSS를 조작하게 해주는 API (어플리케이션 프로그래밍 인터페이스)이다.
    
       => 인터페이스 ?
          구현된 것은 아무것도 없는 밑그림만 있는 기본 설계도
      	  인터페이스는 표준,약속,규칙이다.
    
    * 렌더트리는 오직 스크린에 그려지는 것으로만 구성된다.
    ex) 렌더트리는 p태그를 포함하지 않는다.
    ...
    * 렌더링시 고려해야할(?) 점
    ...
    1.CSS에서 렌더링이 어떤것을 차단하는지 확인
                 
    CSS가 검사되기 전에 브라우저가 웹 페이지의 콘텐츠를 보여줄 경우 스타일이 지정되지 않은 채 표시가된다.
	CSS가 적용되지 않은 웹페이지는 끔찍하다.
                 
	2. 새 페이지를 로드 할 때마다 CSSOM을 빌드해야 한다는 것을 확인
                 
	CSS가 모두 로드되었다고 해서 CSSOM이 모든 페이지에 빌드되는 것은 아니다.
	즉, CSS가 캐시가 되어도 CCSOM이 모든 페이지에 적용되는 것이 아니다.
	따라서, 다른 페이지를 이동할 때마다 CSSOM을 만들어야한다.
    
	3. 페이지가 로드되는 CSS와 페이지가 로드되는 자바스크립트 사이에는 관계가 있다.
                 
	Javascript가 웹 페이지에서 사용되는 방식은 CSSOM을 빌드하지 못하도록 차단할 수 있다.
	CSSOM은 무엇이든 표시해야한다.
	CSSOM의 프로세스를 차단하면 CSSOM의 처리 시간이 길어지고, 표시되는 시간이 길어진다.
	자바 스크립트가 CSSOM이 작성되는 것을 차단하고 있다면 사용자는 빈 페이지를 오래 동안보고 있다.
                 
	따라서, 이 관계를 파악한다면 웹 성능을 향상시킬 수 있는 요인을 알아차릴 수 있다.
-----------------------------------------------------------------------------------------    
4) 렌더링 트리에서 레이아웃을 실행하여 각 노드의 기하학적 형태를 계산한다.("BOX-MODEL"을 생성한다.)

	F12 elements 탭
    position border padding content 등의 요소로 이루어진 웹페이지의 기하학적 형태
    
5) 개별 노드를 화면에 페인트한다.(or 레스터화)

* 자바스크립트는 렌더링 엔진이 아닌 자바스크립트엔진이 처리한다.
...
즉, 문서 자체의 특성이라고 할 수 있다.

- 대표적인 속성값으로는 subject,keywords,title,author 등이 있다.
이러한 메타 정보를 활용해 검색엔진에서 검색 시 적절한 페이지를 보여주도록 할 수 있다.
(메타정보를 데이터베이스화하여)
