...
```
정형 데이터는 고정된 필드에 저장된 데이터, 기존 내부 업무 시스템에서 활용하고 있는 데이터, 형태가 있으며 연산이 가능한 데이터로 분석 활용이 용이

비정형 데이터는 정형화된 필드에 저장되어 있지 않은, 즉 형태가 없는 데이터를 뜻한다. 
텍스트, 영상, 이미지 등이 이에 해당된다. 

반정형 데이터는 정형 데이터와 유사하게 형태가 있다. 하지만 연산이 불가능하다. 
커넥티드 카 서비스로 수집하는 데이터는 반정형 데이터가 대부분이다.
따라서 데이터의 규칙성을 파악한 후, 전처리 절차를 통해 정형화시켜 분석에 활용된다. 
자동차에서 수집되는 센서 데이터가 이에 해당됩니다.
```
